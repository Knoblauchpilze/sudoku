...
namespace {
  ...
  pge::MenuShPtr
  generateGameEntry(const std::string& text,
                    ...
                    const std::string& name)
  {
    ...
    pge::menu::MenuContentDesc fd = pge::menu::newTextContent(text, textColor, pge::menu::Alignment::Center);
    fd.hColor = textHColor;

    pge::MenuShPtr m = std::make_shared<pge::Menu>(
      olc::vi2d(),
      olc::vi2d(),
      name,
      bg,
...
namespace pge {
  ...
  SavedGames::SavedGames(unsigned count,
  ...
  void
  SavedGames::generate(MenuShPtr menu) {
    olc::vi2d dims;

    // Generate previous page button.
    m_previous = generateGameEntry("Previous page", olc::VERY_DARK_CORNFLOWER_BLUE, olc::GREY, olc::BLACK, "previous");
    m_previous->setEnabled(false);

    m_previous->setSimpleAction(
      [this](Game& /*g*/) {
    ...
    for (unsigned id = 0u ; id < m_gamesPerPage ; ++id) {
      MenuShPtr m = generateGameEntry("", olc::DARK_CORNFLOWER_BLUE, olc::GREY, olc::BLACK, "game" + std::to_string(id));
      m->setSimpleAction(
        [this, m](Game& /*g*/) {
    ...
    m_next = generateGameEntry("Next page", olc::VERY_DARK_CORNFLOWER_BLUE, olc::GREY, olc::BLACK, "next");
    m_next->setEnabled(false);

    m_next->setSimpleAction(
      [this](Game& /*g*/) {
