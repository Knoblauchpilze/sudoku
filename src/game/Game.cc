...
pge::MenuShPtr generateMenu(const olc::vi2d &pos, const olc::vi2d &size,
                            const std::string &text, const std::string &name,
                            const olc::Pixel &color, bool clickable = false,
  ...
  return std::make_shared<pge::Menu>(
      pos, size, name, pge::menu::newColoredBackground(color), fd,
      pge::menu::Layout::Horizontal, clickable, selectable);
}

pge::MenuShPtr generateMessageBoxMenu(const olc::vi2d &pos,
                                      const olc::vi2d &size,
                                      const std::string &text,
                                      const std::string &name, bool alert) {
  pge::menu::MenuContentDesc fd = pge::menu::newMenuContent(text, "", size);
  ...
  return std::make_shared<pge::Menu>(
      pos, size, name,
      pge::menu::newColoredBackground(alert ? olc::VERY_DARK_RED
                                            : olc::VERY_DARK_GREEN),
      fd, pge::menu::Layout::Horizontal, false, false);
}
...
std::vector<MenuShPtr> Game::generateMenus(float width, float height) {
  ...
  m_menus.status = generateMenu(
      olc::vi2d(), olc::vi2d(width, STATUS_MENU_HEIGHT), "", "status", bg);

  olc::vi2d pos;
  olc::vi2d dims(50, STATUS_MENU_HEIGHT);
  MenuShPtr mLabel =
      generateMenu(pos, dims, "Cell(s):", "cells_label", BUTTON_BG);
  ...
  solve->setSimpleAction([](Game &g) { g.solve(); });
  m_menus.status->addMenu(solve);

  MenuShPtr reset = generateMenu(pos, dims, "Reset", "reset", BUTTON_BG, true);
  reset->setSimpleAction([](Game &g) { g.reset(); });
  m_menus.status->addMenu(reset);

  m_menus.hint =
      generateMenu(olc::vi2d(0, height - STATUS_MENU_HEIGHT),
                   olc::vi2d(width, STATUS_MENU_HEIGHT), "", "hint", bg);
  ...
  m_menus.solve = generateMenu(pos, olc::vi2d(width, STATUS_MENU_HEIGHT),
                               "Solve !", "solve", olc::DARK_APPLE_GREEN, true);
  m_menus.solve->setSimpleAction([](Game &g) { g.solve(); });

  m_menus.back = generateMenu(olc::vi2d(0, height - STATUS_MENU_HEIGHT),
                              olc::vi2d(width, STATUS_MENU_HEIGHT), "Back",
                              "back", olc::VERY_DARK_BIDOOF, true);
  m_menus.back->setSimpleAction([this](Game &g) { g.finish(); });
  ...
  m_menus.solvedAlert.menu = generateMessageBoxMenu(
      ...
      olc::vi2d(300, 150), solverModeSolvedAlert, "solved_alert", false);
  ...
  m_menus.unsolvableAlert.menu = generateMessageBoxMenu(
      olc::vi2d((width - 300.0f) / 2.0f, (height - 150.0f) / 2.0f),
      olc::vi2d(300, 150), solverModeUnsolvableAlert, "unsolvable_alert", true);
...
void Game::performAction(float x, float y, bool erase) {
  ...
  // Also if the puzzle is already solved, do nothing.
  if (m_state.solverStep == SolverStep::Solved) {
    warn("Ignoring click, sudoku is already solved");
    return;
  }
  ...
  if (erase) {
    if (!m_board->put(ux, uy, 0u, sudoku::DigitKind::None)) {
      warn("Failed to erase digit at " + std::to_string(ux) + "x" +
           std::to_string(uy));
    }
  ...
  while (tries < 10u && !put) {
    ...
    // Reset the solver step if needed.
    if (m_board->solved()) {
...
bool Game::step(float /*tDelta*/) {
  ...
  if (m_hint.x >= 0 && m_hint.y >= 0 &&
      elapsed >= utils::toMilliseconds(HINT_DISPLAY_DELAY_MS)) {
    m_hint.active = true;
  }
...
void Game::setMode(const Mode &mode) noexcept {
  m_state.mode = mode;
  m_state.solverStep =
      (mode == Mode::Interactive ? SolverStep::None : SolverStep::Preparing);

  // Update the texts of the alerts.
  m_menus.solvedAlert.menu->setText(mode == Mode::Interactive
                                        ? interactiveModeSolvedAlert
                                        : solverModeSolvedAlert);
  m_menus.unsolvableAlert.menu->setText(mode == Mode::Interactive
...
void Game::clear() { m_board->clear(); }

const sudoku::Board &Game::board() const noexcept { return (*m_board)(); }
...
void Game::setActiveCell(float x, float y) {
  ...
  unsigned digit = b.at(ix, iy, &kind);

  // Do not display hint if the digit is set already.
  if (kind == sudoku::DigitKind::Generated ||
      kind == sudoku::DigitKind::Generated) {
    resetActiveCell();
  }
...
void Game::onDigitPressed(unsigned digit) {
  ...
  // Also if the puzzle is already solved.
  if (m_state.solverStep == SolverStep::Solved) {
    warn("Ignoring digit pressed, sudoku is already solved");
    return;
  }

  if (!m_board->put(m_hint.x, m_hint.y, digit,
                    sudoku::DigitKind::UserGenerated)) {
    warn("Failed to put digit " + std::to_string(digit) + " at " +
         std::to_string(m_hint.x) + "x" + std::to_string(m_hint.y));
  } else {
...
void Game::setDifficultyLevel(const sudoku::Level &level) {
...
void Game::solve() {
  ...
  while (!nodes.empty()) {
    ...
    m_board->put(node.column(), node.row(), node.value(),
...
void Game::updateUIForInteractive() {
  // Update digits count.
  const sudoku::Board &b = (*m_board)();
  ...
  menu::BackgroundDesc missing =
      pge::menu::newColoredBackground(olc::PALE_YELLOW);

  for (unsigned id = 0u; id < m_menus.digits.size(); ++id) {
    std::string txt =
        std::to_string(id + 1u) + "s: " + std::to_string(count[id]);
  ...
  for (unsigned id = 0u; id < m_hint.menus.size(); ++id) {
    ...
    if (!m_hint.active) {
      ...
      m.setEnabled(fit);
      m.setBackground(
          menu::newColoredBackground(fit ? BUTTON_BG : DISABLED_BUTTON_BG));
    }
  }
}

void Game::updateUIForSolver() {
  bool alertStillOn =
  ...
  if (m_state.mode == Mode::Interactive &&
      m_state.solverStep == SolverStep::Solved && !alertStillOn) {
