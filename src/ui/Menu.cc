...
namespace pge {
  ...
  void
  Menu::setAction(menu::RegisterAction action) {
    m_callback = action;
  }

  void
  Menu::setSimpleAction(action::Process process) {
  ...
  Menu::renderSelf(olc::PixelGameEngine* pge) const {
    ...
    if (m_fg.text == "" && m_fgSprite != nullptr) {
      ...
      // to display.
      olc::vi2d p(
        static_cast<int>(ap.x + m_size.x / 2.0f - m_fg.size.x / 2.0f),
    ...
    switch (m_fg.order) {
      ...
      default:
        switch (m_fg.align) {
          case menu::Alignment::Center:
            tp = olc::vi2d(
              static_cast<int>(ap.x + (m_size.x - cs.x) / 2.0f + m_fg.size.x),
              static_cast<int>(ap.y + (m_size.y - ts.y) / 2.0f)
            );
  ...
  Menu::updateChildren() {
    ...
    for (unsigned id = 0u ; id < m_children.size() ; ++id) {
      if (!m_children[id]->m_fg.expand) {
        expandableSize -= (
          m_layout == menu::Layout::Horizontal ?
